...
MultiHandEyeCalibrator::MultiHandEyeCalibrator(
    const int number_of_waypoints, const int sens_quantity,
    const std::vector<cv::Point3f> object_points, std::vector<cv::Mat> poses,
    const std::vector<cv::Mat> h2e_initial_guess_vec,
    const cv::Mat b2ee_initial_guess,
    ...
    std::vector<std::vector<cv::Mat>> pnp) {
  ...
  pnp_t_vecs_.resize(sens_quantity,
                     std::vector<Eigen::Vector3d>(number_of_waypoints));
  pnp_r_vecs_.resize(sens_quantity,
                     std::vector<Eigen::Vector3d>(number_of_waypoints));
  ...
  for (int i = 0; i < number_of_cameras_; i++) {
    ...
    for (int j = 0; j < number_of_cameras_; j++) {
      if (i != j) {
        ...
        transfMat2Exp<double>(
            h2e_initial_guess_vec[i] * h2e_initial_guess_vec[j].inv(),
            r_vec_c2c, t_vec_c2c);  // j-th camera frame in i-th camera frame
        for (int k = 0; k < 3; k++) {
  ...
  for (auto &cp : object_points) pattern_pts_.emplace_back(cp.x, cp.y, cp.z);
...
void MultiHandEyeCalibrator::eyeOnBaseCalibration(
    const std::vector<CameraInfo> camera_info,
    std::vector<std::vector<std::vector<cv::Point2f>>> corners,
    std::vector<cv::Mat> &optimal_h2e, cv::Mat &optimal_b2ee,
    std::vector<std::vector<cv::Mat>> &optimal_cam2cam) {
  ...
  for (int wp = 0; wp < number_of_waypoints_; wp++) {
    ...
    int detection_amount = accumulate(cross_observation_matrix_[wp].begin(),
                                      cross_observation_matrix_[wp].end(), 0);
    ...
    if (detection_amount > 0) {
      for (int cam = 0; cam < number_of_cameras_; cam++) {
        // Check if the current camera detected the checkerboard in the current
        // pose
        if (cross_observation_matrix_[wp][cam]) {
          ...
          for (int k = 0; k < pattern_pts_.size(); k++) {
            ...
            ceres::CostFunction *cost_function = CalibReprojectionError::Create(
                cam_model_vec[cam], robot_r_vecs_[wp], robot_t_vecs_[wp],
                pattern_pts_[k], observed_pt);
            problem_multi_cam2cam.AddResidualBlock(cost_function, loss_function,
                                                   board2ee_, h2e_[cam]);
          }
        }
      }
      // Multi camera optimization problem whenever the checkerboard is detected
      // simultaneously by more cameras
      if (detection_amount > 1) {
        for (int cam_1 = 0; cam_1 < number_of_cameras_; cam_1++) {
          for (int cam_2 = 0; cam_2 < number_of_cameras_; cam_2++) {
            ...
            if (cam_1 != cam_2 && cross_observation_matrix_[wp][cam_1] &&
                cross_observation_matrix_[wp][cam_2]) {
              ...
              for (int k = 0; k < pattern_pts_.size(); k++) {
                ...
                ceres::CostFunction *cost_function_multi =
                    MultiCalibReprojectionError::Create(
                        cam_model_vec[cam_1], robot_r_vecs_[wp],
                        robot_t_vecs_[wp], pattern_pts_[k], observed_pt);
                problem_multi_cam2cam.AddResidualBlock(
                    cost_function_multi, loss_function, board2ee_, h2e_[cam_2],
                    cam2cam_[cam_1][cam_2]);

                ceres::CostFunction *cost_function_multi_inv =
                    MultiCalibReprojectionErrorInverse::Create(
                        cam_model_vec[cam_2], robot_r_vecs_[wp],
                        robot_t_vecs_[wp], pattern_pts_[k], observed_pt_dir);
                problem_multi_cam2cam.AddResidualBlock(
                    cost_function_multi_inv, loss_function, board2ee_,
                    h2e_[cam_1], cam2cam_[cam_1][cam_2]);
              }
            }
          }
        }
  ...
  for (int cam = 0; cam < number_of_cameras_; cam++) {
    cv::Mat optim_h2e_rvec =
        (cv::Mat_<double>(3, 1) << h2e_[cam][0], h2e_[cam][1], h2e_[cam][2]);
    cv::Mat optim_h2e_tvec =
        (cv::Mat_<double>(3, 1) << h2e_[cam][3], h2e_[cam][4], h2e_[cam][5]);
  ...
  cv::Mat optim_board2tcp_rvec =
      (cv::Mat_<double>(3, 1) << board2ee_[0], board2ee_[1], board2ee_[2]);
  cv::Mat optim_board2tcp_tvec =
      (cv::Mat_<double>(3, 1) << board2ee_[3], board2ee_[4], board2ee_[5]);
  exp2TransfMat<double>(optim_board2tcp_rvec, optim_board2tcp_tvec,
                        optimal_b2ee);
}

void MultiHandEyeCalibrator::eyeInHandCalibration(
    const std::vector<CameraInfo> camera_info,
    std::vector<std::vector<std::vector<cv::Point2f>>> corners,
    std::vector<cv::Mat> &optimal_h2e, cv::Mat &optimal_b2ee,
    std::vector<std::vector<cv::Mat>> &optimal_cam2cam,
        ...
        if (cross_observation_matrix_[wp][cam]) {
          ...
          for (int k = 0; k < pattern_pts_.size(); k++) {
            ...
            ceres::CostFunction *cost_function =
                EyeInHandCalibReprojectionError::Create(
                    cam_model_vec[cam], robot_r_vecs_inv_[wp],
                    robot_t_vecs_inv_[wp], pattern_pts_[k], observed_pt);
            problem_multi_eye_in_hand.AddResidualBlock(
                cost_function, loss_function, board2ee_, h2e_[cam]);
          }
        }
      }
      // Multi camera optimization problem whenever the checkerboard is detected
          ...
          for (int cam_2 = 0; cam_2 < number_of_cameras_; cam_2++) {
            ...
            if (cam_1 != cam_2 && cross_observation_matrix_[wp][cam_1] &&
              ...
              for (int k = 0; k < pattern_pts_.size(); k++) {
                ...
                // MULTI
                ceres::CostFunction *cost_function_multi =
                    MultiEyeInHandCalibReprojectionError::Create(
                        cam_model_vec[cam_1], robot_r_vecs_inv_[wp],
                        robot_t_vecs_inv_[wp], pattern_pts_[k], observed_pt);
                problem_multi_eye_in_hand.AddResidualBlock(
                    cost_function_multi, loss_function, board2ee_, h2e_[cam_2],
                    cam2cam_[cam_1][cam_2]);
                ...
                ceres::CostFunction *cost_function_multi_inv =
                    MultiEyeInHandCalibReprojectionErrorInverse::Create(
                        cam_model_vec[cam_2], robot_r_vecs_inv_[wp],
                        robot_t_vecs_inv_[wp], pattern_pts_[k],
                        observed_pt_dir);
                problem_multi_eye_in_hand.AddResidualBlock(
                    cost_function_multi_inv, loss_function, board2ee_,
                    h2e_[cam_1], cam2cam_[cam_1][cam_2]);
