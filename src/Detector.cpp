...
Detector::Detector(const CalibrationInfo calib_info,
                   const std::vector<CameraInfo> &camera_network_info,
                   const int number_of_waypoints) {
  ...
  if (calib_info_.getPatternType() == "checkerboard") {
    std::cout << "The calibration pattern is the "
              << calib_info_.getPatternType() << ". " << std::endl;
  } else {
    throw std::invalid_argument("The calibration pattern \"" +
                                calib_info_.getPatternType() +
                                "\" is not accepted!");
  }
}
  ...
  if (calib_info_.getPatternType() == "checkerboard") {
    for (int i = 0; i < calib_info_.getNumCol(); ++i) {
      for (int j = 0; j < calib_info_.getNumRow(); ++j) {
        objectPoints.push_back(cv::Point3f(float(j * calib_info_.getSize()),
                                           float(i * calib_info_.getSize()),
                                           0));
      }
    }
  }
...
// If you want to add other patterns, you can do it here by adding another
// function for the pattern detection
void Detector::patternDetection(
  ...
  if (calib_info_.getPatternType() == "checkerboard") {
    checkerboardDetection(images, poses, correct_images, correct_poses,
                          correct_corners_only, correct_corners,
                          cross_observation_matrix, rvec_all, tvec_all);
  }
}

void Detector::checkerboardDetection(
    std::vector<std::vector<cv::Mat>> images,
    std::vector<std::vector<cv::Mat>> poses,
    std::vector<std::vector<cv::Mat>> &correct_images,
    std::vector<std::vector<cv::Mat>> &correct_poses,
  ...
  for (int cam = 0; cam < calib_info_.getNumberOfCams(); cam++) {
    ...
    for (int img = 0; img < number_of_images; img++) {
      ...
      bool pattern_found = cv::findChessboardCorners(
          gray_image, pattern_size_, corners,
      ...
      if (pattern_found) {
        ...
        // RANSAC filter
        cv::Mat rvec, tvec;
        bool correct_detection = filterRansac(
            object_points, corners, camera_network_info_[cam], rvec, tvec);
    ...
    std::cout << "Camera " << std::to_string(cam + 1) << ": process completed, "
              << std::to_string(correct_corners_only[cam].size())
              << " filtered images!" << std::endl;
  }
}
